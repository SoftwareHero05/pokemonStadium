...
int main() {
    try {
        ...
        Pokemon dragonite("./assets/images/149.png", 1.5f, 1.5f, 75, 200);
        Pokemon gengar ("./assets/images/094.png", 1.5f, 1.5f, 300, 70);
        Ataque ataqueDragonite("./assets/images/00013.png", 90, 300, 0.2f, 0.2f); // Imagen del ataque
        Ataque ataqueGengar ("./assets/images/00011.png", 300, 70, 0.2f, 0.2f); // Imagen del ataque de Garchomp
        
        bool dragoniteVisible = false;
        bool gengarVisible = false;
        ...
        while (window.isOpen()) {
            ...
            while (window.pollEvent(event)) {
                ...
                if (event.type == sf::Event::KeyPressed && event.key.code == sf::Keyboard::M) {
                    if (!ataqueDragonite.estaActivo()) { // Lanza el ataque solo si no está activo
                        ataqueDragonite.lanzar(230, 200, 400, 110); // Desde Gardevoir a Garchomp
                    }
                }

                if (event.type == sf::Event::KeyPressed && event.key.code == sf::Keyboard::N) {
                    if (!ataqueGengar .estaActivo()) { // Lanza el ataque solo si no está activo
                        ataqueGengar .lanzar(400, 80, 85, 320); // Desde Garchomp a Gardevoir
                    }
                } 

            }
            ataqueDragonite.actualizar(0.2f);
            ataqueGengar .actualizar(0.2f);
            ...
            ataqueDragonite.draw(window);
            ataqueGengar .draw(window);
            window.display();
        }
